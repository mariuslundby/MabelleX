...
configure_scep_ca() {
    ...
    # Step 1: Download CA certificate FIRST (required for -R flag)
    echo "=== DOWNLOADING CA CERTIFICATE ==="
    log "Downloading CA certificate for -R flag..."

    # Method 1: Use scep-submit to download CA cert
    if timeout 30 $SCEP_HELPER -u "$SCEP_URL" -C /tmp/ndes-ca.crt 2>&1; then
        if [ -s /tmp/ndes-ca.crt ]; then
            log_success "Downloaded CA cert via scep-submit ($(wc -c < /tmp/ndes-ca.crt) bytes)"
        else
            log_warn "scep-submit did not produce CA cert file"
        fi
    else
        log_warn "scep-submit failed to download CA cert"
    fi

    # Method 2: If method 1 failed, try GetCACert operation
    if [ ! -s /tmp/ndes-ca.crt ]; then
        ...
        if timeout 30 curl -k -s "$GETCACERT_URL" -o /tmp/ndes-ca.p7b 2>&1 && [ -s /tmp/ndes-ca.p7b ]; then
            if openssl pkcs7 -in /tmp/ndes-ca.p7b -inform DER -print_certs -out /tmp/ndes-ca.crt 2>&1; then
                if [ -s /tmp/ndes-ca.crt ]; then
                    log_success "Downloaded CA via GetCACert ($(wc -c < /tmp/ndes-ca.crt) bytes)"
                fi
            fi
        fi
    fi

    # Verify we have CA cert
    if [ ! -s /tmp/ndes-ca.crt ]; then
        log_error "Failed to download CA certificate - cannot proceed"
        log_error "HTTPS URLs require -R flag with CA certificate"
        return 1
    fi
    ...
    if [ -d /etc/pki/ca-trust/source/anchors ]; then
        ...
        log_success "Installed CA in system trust (RHEL/Fedora/Rocky/CentOS)"
    elif [ -d /usr/local/share/ca-certificates ]; then
        cp /tmp/ndes-ca.crt /usr/local/share/ca-certificates/ndes-ca.crt
        update-ca-certificates 2>&1 | head -3
        log_success "Installed CA in system trust (Debian/Ubuntu)"
    fi
    sleep 3

    # Step 2: Add SCEP CA with -R flag (required for HTTPS)
    echo "=== ADDING SCEP CA WITH -R FLAG ==="
    log "Running: getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R /tmp/ndes-ca.crt"
    ...
    # Step 3: Wait longer and verify CA was added
    echo "=== VERIFYING CA ==="
    log "Waiting for CA to be configured..."
    sleep 10
    ...
    if ! echo "$CA_LIST" | grep -q "^CA '$CA_NAME'"; then
        log_error "CA '$CA_NAME' was not added to certmonger"
        echo "--- CA LIST ---"
        echo "$CA_LIST"
        echo "--- END CA LIST ---"
        return 1
    ...
    # Step 4: Verify CA is actually configured (not in error state)
    echo "=== CA CONFIGURATION DETAILS ==="
    CA_CONFIG=$(getcert list-cas -c "$CA_NAME" 2>&1)
    echo "$CA_CONFIG"

    # Check for common error indicators in CA config
    if echo "$CA_CONFIG" | grep -qi "error\|failed\|invalid"; then
        log_error "CA configuration contains errors"
        return 1
    fi

    log_success "SCEP CA '$CA_NAME' successfully configured"
    record_state "CA_ADDED:$CA_NAME"

    # Extra wait to ensure CA is fully ready
    log "Waiting 5 more seconds for CA to stabilize..."
    sleep 5
...
if [ "$TEST_MODE" = true ]; then
    ...
    log "Waiting for certmonger to start..."
    sleep 10
