...
log() {
    ...
    # Also echo to stdout for ManageEngine
    echo "[LOG] $1"
}
...
configure_scep_ca() {
    ...
    # Test reachability
    echo "=== TESTING SCEP SERVER REACHABILITY ==="
    if timeout 10 curl -k -s -I "$SCEP_URL" >/dev/null 2>&1; then
        log_success "SCEP server is reachable"
    else
        log_error "Cannot reach SCEP server: $SCEP_URL"
        return 1
    fi

    # Download CA certs via GetCACert
    echo "=== DOWNLOADING SCEP CA CERTIFICATES ==="
    GETCACERT_URL="${SCEP_URL}?operation=GetCACert&message=CA"

    if timeout 30 curl -k -s "$GETCACERT_URL" -o /tmp/scep-ca-bundle.p7b 2>&1 && [ -s /tmp/scep-ca-bundle.p7b ]; then
        if openssl pkcs7 -in /tmp/scep-ca-bundle.p7b -inform DER -print_certs -out /tmp/scep-ca-chain.pem 2>&1 && [ -s /tmp/scep-ca-chain.pem ]; then
            log_success "Downloaded SCEP CA certificate chain"

            # Install in system trust
            if [ -d /etc/pki/ca-trust/source/anchors ]; then
                cp /tmp/scep-ca-chain.pem /etc/pki/ca-trust/source/anchors/scep-ca.crt
                update-ca-trust extract 2>&1
                log_success "Installed in system trust (RHEL/Fedora)"
            elif [ -d /usr/local/share/ca-certificates ]; then
                cp /tmp/scep-ca-chain.pem /usr/local/share/ca-certificates/scep-ca.crt
                update-ca-certificates 2>&1
                log_success "Installed in system trust (Debian)"
            fi
            sleep 3
        fi
    else
        log_warn "Could not download via GetCACert, trying TLS cert..."
        if timeout 10 openssl s_client -connect "${SCEP_HOST}:443" -showcerts </dev/null 2>/dev/null |             sed -n '/BEGIN CERTIFICATE/,/END CERTIFICATE/p' > /tmp/scep-tls-ca.pem 2>&1 && [ -s /tmp/scep-tls-ca.pem ]; then

            if [ -d /etc/pki/ca-trust/source/anchors ]; then
                cp /tmp/scep-tls-ca.pem /etc/pki/ca-trust/source/anchors/scep-tls.crt
                update-ca-trust extract 2>&1
                log_success "Installed TLS CA in system trust (RHEL/Fedora)"
            elif [ -d /usr/local/share/ca-certificates ]; then
                cp /tmp/scep-tls-ca.pem /usr/local/share/ca-certificates/scep-tls.crt
                update-ca-certificates 2>&1
                log_success "Installed TLS CA in system trust (Debian)"
            fi
            sleep 3
        fi
    fi

    # Now add SCEP CA to certmonger
    echo "=== ADDING SCEP CA TO CERTMONGER ==="
    log "Running: getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL""

    SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" 2>&1)
    SCEP_EXIT=$?

    echo "--- getcert add-scep-ca OUTPUT START ---"
    echo "$SCEP_OUTPUT"
    echo "--- getcert add-scep-ca OUTPUT END ---"
    echo "Exit code: $SCEP_EXIT"

    log_debug "Exit code: $SCEP_EXIT"
    log_debug "Output: $SCEP_OUTPUT"

    # Wait and verify
    echo "=== VERIFYING CA WAS ADDED ==="
    sleep 5

    CA_LIST=$(getcert list-cas 2>&1)
    echo "--- getcert list-cas OUTPUT START ---"
    echo "$CA_LIST"
    echo "--- getcert list-cas OUTPUT END ---"

    if echo "$CA_LIST" | grep -q "^CA '$CA_NAME'"; then
        log_success "SCEP CA '$CA_NAME' successfully added!"
        record_state "CA_ADDED:$CA_NAME"
        return 0
    else
        log_error "CA '$CA_NAME' NOT FOUND in certmonger list"
        log_error "This means getcert add-scep-ca silently failed"

        # Try with -R flag if we have cert
        if [ -f /tmp/scep-ca-chain.pem ]; then
            echo "=== RETRY WITH -R FLAG ==="
            getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
            sleep 2

            SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R /tmp/scep-ca-chain.pem 2>&1)
            echo "--- RETRY OUTPUT ---"
            echo "$SCEP_OUTPUT"
            echo "--- END RETRY OUTPUT ---"

            sleep 5
            if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
                log_success "Success with -R flag"
                record_state "CA_ADDED:$CA_NAME"
                return 0
            fi
        fi
        ...
        # Try HTTP
        SCEP_URL_HTTP=$(echo "$SCEP_URL" | sed 's|https://|http://|')
        if [[ "$SCEP_URL_HTTP" != "$SCEP_URL" ]]; then
            echo "=== RETRY WITH HTTP ==="
            getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
            sleep 2

            SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL_HTTP" 2>&1)
            echo "--- HTTP OUTPUT ---"
            echo "$SCEP_OUTPUT"
            echo "--- END HTTP OUTPUT ---"

            sleep 5
            if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
                log_success "Success with HTTP"
                record_state "CA_ADDED:$CA_NAME"
                SCEP_URL="$SCEP_URL_HTTP"
                return 0
            fi
        fi

        log_error "ALL METHODS FAILED"
        log_error "Manual debug: getcert add-scep-ca -c TEST -u $SCEP_URL -v"
        return 1
    fi
}
...
if [ "$TEST_MODE" = true ]; then
    ...
    if getcert list &>/dev/null; then
        log_success "Certmonger is ready"
    else
        log_error "Certmonger is not responding"
