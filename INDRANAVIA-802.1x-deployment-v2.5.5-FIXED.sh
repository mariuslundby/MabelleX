...
configure_scep_ca() {
    ...
    # Remove existing CA if present (idempotent)
    ...
    # Detect OS-specific SCEP helper path
    local SCEP_HELPER=""
    if [ -f /usr/libexec/certmonger/scep-submit ]; then
        SCEP_HELPER="/usr/libexec/certmonger/scep-submit"
        log_debug "Using SCEP helper: $SCEP_HELPER (RHEL/Fedora)"
    elif [ -f /usr/lib/certmonger/scep-submit ]; then
        SCEP_HELPER="/usr/lib/certmonger/scep-submit"
        log_debug "Using SCEP helper: $SCEP_HELPER (Debian/Ubuntu)"
    else
        log_error "SCEP helper not found"
        return 1
    fi

    # Install NDES server TLS certificate in system trust store
    log "Installing NDES server CA in system trust store..."
    SCEP_HOST=$(echo "$SCEP_URL" | sed 's|https://||;s|/.*||')

    if timeout 10 openssl s_client -connect "${SCEP_HOST}:443" -showcerts </dev/null 2>/dev/null |         openssl x509 -outform PEM > /tmp/ndes-tls-ca.pem 2>/dev/null && [ -s /tmp/ndes-tls-ca.pem ]; then

        # Install in system CA trust store (Fedora/RHEL)
        if [ -d /etc/pki/ca-trust/source/anchors ]; then
            cp /tmp/ndes-tls-ca.pem /etc/pki/ca-trust/source/anchors/ndes-server.crt
            update-ca-trust extract
            log_success "NDES server CA installed in system trust store (RHEL)"
        elif [ -d /usr/local/share/ca-certificates ]; then
            # Debian/Ubuntu
            cp /tmp/ndes-tls-ca.pem /usr/local/share/ca-certificates/ndes-server.crt
            update-ca-certificates
            log_success "NDES server CA installed in system trust store (Debian)"
        fi
        sleep 3
    else
        log_warn "Could not download NDES server TLS certificate"
    fi

    # Method 1: Try add-scep-ca without -R flag (let system trust store handle it)
    log "Attempting SCEP CA configuration (method 1: using system trust)..."
    SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" 2>&1)
    log_debug "SCEP CA output: $SCEP_OUTPUT"
    echo "$SCEP_OUTPUT" >> "$LOG_FILE"

    sleep 5
    if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
        log_success "SCEP CA configured successfully (method 1: system trust)"
        record_state "CA_ADDED:$CA_NAME"
        return 0
    else
        log_warn "Method 1 failed - CA not found in list"
        getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    fi

    # Method 2: Try with -R flag and downloaded cert
    if [ -f /tmp/ndes-tls-ca.pem ]; then
        log_warn "Trying method 2: add-scep-ca with -R flag..."
        sleep 2

        SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R /tmp/ndes-tls-ca.pem 2>&1)
        log_debug "SCEP CA output (method 2): $SCEP_OUTPUT"
        echo "$SCEP_OUTPUT" >> "$LOG_FILE"

        sleep 5
        if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
            log_success "SCEP CA configured successfully (method 2: with -R)"
            record_state "CA_ADDED:$CA_NAME"
            return 0
        else
            log_warn "Method 2 failed - CA not found in list"
            getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
        fi
    fi

    # Method 3: Try HTTP fallback
    SCEP_URL_HTTP=$(echo "$SCEP_URL" | sed 's|https://|http://|')
    if [[ "$SCEP_URL_HTTP" != "$SCEP_URL" ]]; then
        log_warn "Trying method 3: HTTP instead of HTTPS..."
        sleep 2

        SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL_HTTP" 2>&1)
        log_debug "SCEP CA output (method 3): $SCEP_OUTPUT"
        echo "$SCEP_OUTPUT" >> "$LOG_FILE"

        sleep 5
        if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
            log_success "SCEP CA configured successfully (method 3: HTTP)"
            record_state "CA_ADDED:$CA_NAME"
            SCEP_URL="$SCEP_URL_HTTP"
            return 0
        else
            log_warn "Method 3 failed - CA not found in list"
        fi
    fi

    log_error "Failed to configure SCEP CA after trying all methods"
    log_error "Check logs: journalctl -u certmonger -n 50"
    log_error "Check system trust store: ls -la /etc/pki/ca-trust/source/anchors/"
    return 1
}
...
if [ "$TEST_MODE" = true ]; then
    ...
    # Wait for certmonger to fully start
    sleep 10

    # Restart certmonger to ensure clean state
