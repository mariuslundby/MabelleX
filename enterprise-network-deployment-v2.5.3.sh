...
configure_scep_ca() {
    ...
    # Download SCEP server TLS CA certificate first (required for HTTPS)
    if ! download_scep_server_ca; then
        log_warn "Could not download SCEP server CA, will try without -R flag"
        SCEP_SERVER_CA_CERT=""
    fi
    ...
    if [ -f /usr/libexec/certmonger/scep-submit ]; then
        ...
        log_error "Expected paths: /usr/libexec/certmonger/scep-submit or /usr/lib/certmonger/scep-submit"
        return 1
    fi

    # Try method 1: getcert add-scep-ca with -R flag (HTTPS CA cert)
    if [ -n "$SCEP_SERVER_CA_CERT" ] && [ -f "$SCEP_SERVER_CA_CERT" ]; then
        log "Attempting SCEP CA configuration (method 1: add-scep-ca with -R)..."
        if getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R "$SCEP_SERVER_CA_CERT" 2>&1 | tee -a "$LOG_FILE" | grep -qE "(CA.*added|^$)"; then
            # Verify CA was added
            sleep 2
            if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
                log_success "SCEP CA configured via add-scep-ca with TLS certificate"
    ...
    # Try method 2: Use HTTP instead of HTTPS (fallback)
    SCEP_URL_HTTP=$(echo "$SCEP_URL" | sed 's|https://|http://|')
    if [[ "$SCEP_URL_HTTP" != "$SCEP_URL" ]]; then
        log_warn "Method 1 failed, trying HTTP instead of HTTPS..."
        log_warn "Using: $SCEP_URL_HTTP"

        if getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL_HTTP" 2>&1 | tee -a "$LOG_FILE" | grep -qE "(CA.*added|^$)"; then
            sleep 2
            if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
                log_success "SCEP CA configured via HTTP"
                record_state "CA_ADDED:$CA_NAME"
                # Update SCEP_URL for future use
    ...
    log_error "Failed to configure SCEP CA after trying multiple methods"
    log_error ""
    log_error "Manual troubleshooting:"
    log_error "  1. Check certmonger is running: systemctl status certmonger"
    log_error "  2. Check existing CAs: getcert list-cas"
    log_error "  3. Check if SCEP server is reachable:"
    log_error "     curl -I $SCEP_URL"
    log_error "  4. Try with HTTP: getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL_HTTP""
    log_error "  5. Check logs: journalctl -u certmonger -n 50"
