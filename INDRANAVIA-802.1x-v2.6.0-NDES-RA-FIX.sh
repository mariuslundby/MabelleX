...
configure_scep_ca() {
    ...
    # Step 1: Download CA certificate bundle from SCEP server
    echo "=== DOWNLOADING SCEP CERTIFICATES ==="
    log "Downloading SCEP CA certificate bundle..."

    # Use scep-submit to download CA cert
    if timeout 30 $SCEP_HELPER -u "$SCEP_URL" -C /tmp/ndes-ca-bundle.crt 2>&1 | tee -a "$LOG_FILE"; then
        if [ -s /tmp/ndes-ca-bundle.crt ]; then
            log_success "Downloaded CA bundle ($(wc -c < /tmp/ndes-ca-bundle.crt) bytes)"
            echo "Certificate bundle preview:"
            head -20 /tmp/ndes-ca-bundle.crt
        fi
    else
        log_warn "scep-submit failed, trying GetCACert..."

        # Fallback: GetCACert operation
        GETCACERT_URL="${SCEP_URL}?operation=GetCACert&message=CA"
        if timeout 30 curl -k -s "$GETCACERT_URL" -o /tmp/ndes-ca.p7b 2>&1; then
            if [ -s /tmp/ndes-ca.p7b ]; then
                openssl pkcs7 -in /tmp/ndes-ca.p7b -inform DER -print_certs -out /tmp/ndes-ca-bundle.crt 2>&1
                if [ -s /tmp/ndes-ca-bundle.crt ]; then
                    log_success "Downloaded CA via GetCACert ($(wc -c < /tmp/ndes-ca-bundle.crt) bytes)"
                fi
            fi
        fi
    fi
    ...
    if [ ! -s /tmp/ndes-ca-bundle.crt ]; then
        log_error "Failed to download CA certificate bundle"
        return 1
    fi

    # Step 2: Split certificate bundle into RA cert and CA cert
    echo "=== SPLITTING CERTIFICATE BUNDLE ==="
    log "Splitting RA and CA certificates..."

    # Extract first certificate (RA cert)
    csplit -s -f /tmp/cert- /tmp/ndes-ca-bundle.crt '/-----BEGIN CERTIFICATE-----/' '{*}'

    # First non-empty file is RA cert
    RA_CERT=""
    CA_CERT=""
    for certfile in /tmp/cert-*; do
        if [ -s "$certfile" ] && grep -q "BEGIN CERTIFICATE" "$certfile"; then
            if [ -z "$RA_CERT" ]; then
                RA_CERT="$certfile"
                cp "$certfile" /tmp/ndes-ra.crt
                log_success "Extracted RA certificate"
            else
                # Append to CA chain
                cat "$certfile" >> /tmp/ndes-ca-chain.crt
            fi
        fi
    done

    # If split didn't work, use different approach
    if [ ! -s /tmp/ndes-ra.crt ] || [ ! -s /tmp/ndes-ca-chain.crt ]; then
        log_warn "Certificate split failed, using bundle as-is"
        cp /tmp/ndes-ca-bundle.crt /tmp/ndes-ra.crt
        cp /tmp/ndes-ca-bundle.crt /tmp/ndes-ca-chain.crt
    else
        log_success "Split into RA cert ($(wc -c < /tmp/ndes-ra.crt) bytes) and CA chain ($(wc -c < /tmp/ndes-ca-chain.crt) bytes)"
    fi

    # Step 3: Install CA certificates in system trust store
    echo "=== INSTALLING IN SYSTEM TRUST STORE ==="
    if [ -d /etc/pki/ca-trust/source/anchors ]; then
        cp /tmp/ndes-ca-chain.crt /etc/pki/ca-trust/source/anchors/ndes-ca.crt 2>/dev/null
        cp /tmp/ndes-ra.crt /etc/pki/ca-trust/source/anchors/ndes-ra.crt 2>/dev/null
        update-ca-trust extract 2>&1 | head -3
        log_success "Installed in system trust (RHEL/Fedora/Rocky/CentOS)"
    elif [ -d /usr/local/share/ca-certificates ]; then
        cp /tmp/ndes-ca-chain.crt /usr/local/share/ca-certificates/ndes-ca.crt 2>/dev/null
        cp /tmp/ndes-ra.crt /usr/local/share/ca-certificates/ndes-ra.crt 2>/dev/null
        update-ca-certificates 2>&1 | head -3
        log_success "Installed in system trust (Debian/Ubuntu)"
    fi
    sleep 5

    # Step 4: Add SCEP CA with both -R (CA cert) and -r (RA cert) flags
    echo "=== ADDING SCEP CA TO CERTMONGER ==="
    log "Adding SCEP CA with RA certificate..."

    # Try with both -R and -r flags (NDES requires both)
    log "Method 1: With -R (CA) and -r (RA) flags..."
    SCEP_CMD="getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R /tmp/ndes-ca-chain.crt -r /tmp/ndes-ra.crt"
    echo "Running: $SCEP_CMD"

    SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R /tmp/ndes-ca-chain.crt -r /tmp/ndes-ra.crt 2>&1)
    SCEP_EXIT=$?

    echo "--- OUTPUT ---"
    echo "$SCEP_OUTPUT"
    echo "--- END OUTPUT ---"
    echo "Exit code: $SCEP_EXIT"

    sleep 10

    # Verify CA was added
    echo "=== VERIFYING CA ==="
    CA_LIST=$(getcert list-cas 2>&1)

    if ! echo "$CA_LIST" | grep -q "^CA '$CA_NAME'"; then
        log_error "CA not found after add-scep-ca"

        # Retry without -r flag
        log_warn "Retrying without -r flag..."
        getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
        sleep 3

        SCEP_OUTPUT=$(getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL" -R /tmp/ndes-ca-bundle.crt 2>&1)
        echo "Retry output:"
        echo "$SCEP_OUTPUT"
        ...
        if ! getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
            log_error "CA still not found after retry"
            echo "--- CA LIST ---"
            echo "$CA_LIST"
            return 1
        fi
    ...
    log_success "CA '$CA_NAME' found in certmonger"

    # Show CA configuration
    echo "=== CA CONFIGURATION ==="
    getcert list-cas -c "$CA_NAME" 2>&1

    log_success "SCEP CA configured successfully"
    record_state "CA_ADDED:$CA_NAME"

    # Extra wait for CA to be fully operational
    log "Waiting 10 seconds for CA to be fully operational..."
    sleep 10
...
if [ "$TEST_MODE" = true ]; then
    ...
    log "Waiting for certmonger..."
    sleep 10

    log "Restarting certmonger..."
