...
configure_scep_ca() {
    ...
    if timeout 10 openssl s_client -connect "${SCEP_HOST}:443" -showcerts </dev/null 2>/dev/null |         openssl x509 -outform PEM > /tmp/ndes-tls-ca.pem 2>/dev/null && [ -s /tmp/ndes-tls-ca.pem ]; then
        ...
        if [ -d /etc/pki/ca-trust/source/anchors ]; then
            ...
            log_success "NDES server CA installed in system trust store"
        elif [ -d /etc/ca-certificates ]; then
            # Debian/Ubuntu
    ...
    # Method 1: Try add-scep-ca without -R flag (let system handle TLS)
    log "Attempting SCEP CA configuration (method 1: system trust)..."
    ...
    # Method 2: Try with direct helper
    log_warn "Method 1 failed, trying with direct CA helper..."
    getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
    sleep 2

    if getcert add-ca -c "$CA_NAME" -e "$SCEP_HELPER" "$SCEP_URL" 2>&1 | tee -a "$LOG_FILE"; then
        sleep 3
        if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
            log_success "SCEP CA configured with direct helper"
            record_state "CA_ADDED:$CA_NAME"
            return 0
        fi
    fi
    ...
    if [[ "$SCEP_URL_HTTP" != "$SCEP_URL" ]]; then
        log_warn "HTTPS methods failed, trying HTTP..."
        getcert remove-ca -c "$CA_NAME" 2>/dev/null || true
        sleep 2

        if getcert add-scep-ca -c "$CA_NAME" -u "$SCEP_URL_HTTP" 2>&1 | tee -a "$LOG_FILE"; then
            sleep 3
            if getcert list-cas 2>/dev/null | grep -q "^CA '$CA_NAME'"; then
                log_success "SCEP CA configured via HTTP"
                record_state "CA_ADDED:$CA_NAME"
